...
var getTokenResponse = function (query) {
  ...
  try {
    responseContent = Meteor.http.post(
      "https://us.battle.net/oauth/token?grant_type=authorization_code&code=" + query.code + "&redirect_uri=" + Meteor.absoluteUrl("_oauth/linkedin?close") + "&client_id=" + config.clientId + "&client_secret=" + OAuth.openSecret(config.secret)
     ).content;
  } catch (err) {
